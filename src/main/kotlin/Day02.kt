...
To get information, once a bag has been loaded with cubes, the Elf will reach into the bag, grab a handful of random
 cubes, show them to you, and then put them back in the bag.
...
The first set is 3 blue cubes and 4 red cubes; the second set is 1 red cube, 2 green cubes, and 6 blue cubes; the third
...
He isn't sure why the water stopped; however, he can show you how to get to the water source to check it out for
 yourself.
It's just up ahead!
...
private data class GameSet(
    ...
    fun validate(maxR: Int, maxG: Int, maxB: Int): Boolean {
        //Validate that a GameSet is possible given a maximum number of each cube
        return (this.r <= maxR) && (this.g <= maxG) && (this.b <= maxB)
    }
}

private fun String.toGameSet(): GameSet {
    //Parse a GameSet from a string.
    // String must be in format like `6 red, 1 blue, 3 green`
    val map = hashMapOf(
        Pair("red", 0), Pair("green", 0), Pair("blue", 0)
    )
    this.split(", ").map {
        val (valueString, colourString) = it.split(" ")
        map[colourString] = valueString.toInt()
    }
    return GameSet(map["red"]!!, map["green"]!!, map["blue"]!!)
}

private data class Game(
    val id: Int, val sets: List<GameSet>
) {
    fun validate(maxR: Int, maxG: Int, maxB: Int): Boolean {
        //Validate that the game is valid given a max number of each cube
        // A game is valid if each game set is valid
        return this.sets.all { it.validate(maxR, maxG, maxB) }
    }

    fun cubesRequired(): Triple<Int, Int, Int> {
        // Return the minimum number of each cube required for the full GameSet to be valid.
        return Triple(this.sets.maxOf { it.r }, this.sets.maxOf { it.g }, this.sets.maxOf { it.b })
    }
}

private fun String.toGame(): Game {
    //Parse a Game from a string.
    // String must be in format like `Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green`
    // Discard "Game ". Split into ID and GameSets.

    val (idString, gamesetString) = this.removePrefix("Game ").split(": ")

    //Parse ID
    val id = idString.toInt()
    //Parse GameSets
    val sets = gamesetString.split("; ").map {
        it.toGameSet()
    }
...
class Day02 {
    private fun getInputAsReader(): BufferedReader {
        val inputStream = this::class.java.classLoader.getResourceAsStream("inputs/day02.txt")!!
    ...
    fun solvePartOne(): Int {
        ...
        val result = reader.lines().parallel().map { line ->
            line.toGame()
        }.filter { game ->
            //12 red cubes, 13 green cubes, and 14 blue cubes.
            game.validate(maxR = 12, maxG = 13, maxB = 14)
        }.mapToInt { game ->
            game.id
        }.sum()
    ...
    fun solvePartTwo(): Int {
        ...
        val result = reader.lines().parallel().map { line ->
            ...
            game.cubesRequired()
        }.mapToInt { cubesRequired ->
            cubesRequired.first * cubesRequired.second * cubesRequired.third
        }.sum()
